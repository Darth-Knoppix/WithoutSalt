...
CGINCLUDE
	...
	half4 fragAbsoluteYAndDistance (v2f i) : SV_Target
	{
		...
		return lerp(tex2D(_MainTex, i.uv), _FogColor, (half)ComputeFogForYAndDistance(wsDir.xyz,wsPos.xyz));
	}
	...
	half4 fragRelativeYAndDistance (v2f i) : SV_Target
	{
		float dpth = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture,i.uv_depth));
		float4 wsDir = dpth * i.interpolatedRay;
		return lerp(tex2D(_MainTex, i.uv), _FogColor, (half)ComputeFogForYAndDistance(wsDir.xyz, wsDir.xyz));
	}
	...
	half4 fragAbsoluteY (v2f i) : SV_Target
	{
		...
		fogVert *= fogVert; 
		fogVert = (exp (-fogVert));
		return lerp(tex2D( _MainTex, i.uv ), _FogColor, (half)fogVert);				
	}
